...
_MODEL_PATH = os.path.join(
    ...
    "../../../ml_engine/models/custom_interview_nlp_model.pkl",
)
_nlp_model = None


def get_nlp_model():
    """Lazy-loads the custom NLP model once and caches it."""
    global _nlp_model
    if _nlp_model is None:
        try:
            _nlp_model = joblib.load(_MODEL_PATH)
...
async def transcribe_audio(audio_file: UploadFile) -> str:
    """Uses Groq's Whisper API to transcribe the audio."""
    if not GROQ_API_KEY:
...
def analyze_interview_response(question: str, transcript: str) -> Dict[str, Any]:
    ...
    # 1. Score using locally trained NLP model
    model = get_nlp_model()
    tech_score = 0
    conf_score = 0
    if model:
    ...
    # 2. Get LLM feedback & next question from Groq
    if not GROQ_API_KEY:
        raise ValueError("GROQ_API_KEY is not set.")
    ...
    return {
        ...
        "clarity_score": conf_score,  # mapped from confidence for UI simplicity
